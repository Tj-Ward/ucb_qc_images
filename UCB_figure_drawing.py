...
spec = importlib.util.spec_from_file_location("spm_smooth", '/home/jagust/petcore/qc/scripts/smoothing.py')
...
slice_number_font = ImageFont.truetype(font_path, 24)
...
BRAAK5 = [1028,1012,1014,1032,1003,1027,1018,1019,1020,1011,1031,1008,1030,1029,1025,1001,\
        1034,2028,2012,2014,2032,2003,2027,2018,2019,2020,2011,2031,2008,2030,2029,2025,2001,2034]
...
CORTICAL_SUMMARY_CODES = [1009,2009,1015,1030,2015,2030,1003,1012,1014,1018,1019,1020,1027,1028,1032,2003,2012,2014,\
...
HEMISPHERE_OUTLINE_CODES_PATH = '/home/jagust/petcore/qc/scripts/required/QC_Image_Cortex_LUT.txt'
HEMISPHERE_OUTLINE_CODES = list(pd.read_csv(HEMISPHERE_OUTLINE_CODES_PATH)['Index'].astype(int))
...
def filter_gaussian(arr, affine ,fwhm):
    ...
    gauss = np.copy(arr)
    #gauss[nan_mask] = 0
    gauss = gaussian_filter(gauss, sigma=sigma, order=0, mode='nearest', cval=0,truncate=10)
    #gauss[nan_mask] = 0
...
def loadImage(image,colormap,scale,alpha=1,resample=False):
    ...
    image_shape = image.shape
    if resample:
        image = resample_img(image,target_affine=A,\
...
def loadAparc(image,alpha=1,indicies=False,resample=False):
    ...
    A=np.array([[ -1.,   0.,   0., 127.],
       ...
       [  0.,   0.,   0.,   1.]])
    target_shape=[256,256,256]
    color_LUT_dict = pd.read_csv('/home/jagust/petcore/qc/scripts/required/FreeSurferColorLUT_brain.txt')#.set_index('Index')
    color_LUT_dict[['R','G','B','A']] = color_LUT_dict[['R','G','B','A']]#/256
    color_LUT_dict['A'] = [255,]*len(color_LUT_dict['A'])
    color_LUT_dict.loc[0,:]= [0,'Unknown',0,0,0,0]
    color_LUT_dict.loc[len(color_LUT_dict)] = [999999,'Error',0,0,0,0]
    color_LUT_dict = color_LUT_dict[['Index','R','G','B','A']].set_index('Index').astype(int).T.to_dict('list')
    ...
    image_array = np.array([color_LUT_dict[x] for x in u])[inv].reshape(image_array.shape+(4,))
...
def draw_mask(image_2D,outline_value=0,flip_LR=False):
    ...
    if outline_value != 0:
        ...
        for c in cnts:
            cv2.drawContours(omask, [c], -1, (255,255,255,255), thickness=outline_value)
...
def draw_mrifree(output,pet,tracer,display_path_name=False,image_date=False):
    ...
    if tracer in CEREBGM+WCEREB:
        ...
        print('Tracer (', tracer,')','does not have a reference region defined in this code!' )
    ...
    cropped_voxels = (slice_xyz[0].indices(pet_img.shape[0])[0] , 
    ...
    pet_nih_noalpha_img = loadImage(pet_img,NIH,(MIN,MAX),alpha=0.95,resample=False)

    # Create aparc RGBA image for use in cortical ribon images
    
    QC_cortex_outline = loadMask(aparc_img,rgba=(255,255,255,255),indicies=BRAIN_OUTLINE_CODES)
    ...
    if tracer in METAROI_TRACERS:
        primary_roi_outline = loadMask(aparc_img,rgba=(255,255,255,255),indicies=META_TEMPORAL_CODES)
    ...
    axial_images = ImageOps.scale(axial_images, (QC_IMAGE_WIDTH/axial_images.size[0]), resample=Image.LANCZOS)
    ...
    for i,number in enumerate(Display_Axial_Slices):
        ...
        axial_images = addText(axial_images,number,(x_position,0),font=slice_number_font)

    
    border_between_groups = cm.gray(np.repeat(np.zeros(QC_IMAGE_WIDTH).reshape(1,-1),50,0),bytes=True)
    scan_summary = np.concatenate([border_between_groups,axial_images,],axis=0)
    ...
    def draw_bottom_rows(Slices,Display_Slices,pet,masks,axis):
        ...
        row = ImageOps.scale(row, (QC_IMAGE_WIDTH/row.size[0]), resample=Image.LANCZOS)
    ...
    r1 = draw_bottom_rows(Axial_Slices,
                          Display_Axial_Slices,
                          pet_nih_noalpha_img,
                          [QC_cortex_outline,],
                          axis=2)
    
    r2 = draw_bottom_rows(Coronal_Slices,
                          Display_Coronal_Slices,
                          pet_nih_noalpha_img,
                          [QC_cortex_outline,],
                          axis=1)
    r3 = draw_bottom_rows(Sagittal_Slices,
                          Display_Sagittal_Slices,
                          pet_nih_noalpha_img,
                          [QC_cortex_outline,],
                          axis=0)
    
    scan_summary = np.concatenate([scan_summary,border_between_groups,r1,r2,r3],axis=0)
    ...
    r1 = draw_bottom_rows(Axial_Slices,
                          Display_Axial_Slices,
                          pet_nih_noalpha_img,
                          [primary_roi_outline,reference_region_img_filled,],
                          axis=2)
    r2 = draw_bottom_rows(Coronal_Slices,
                          Display_Coronal_Slices,
                          pet_nih_noalpha_img,
    ...
    cbar_height = 30
    cbar = NIH_noalpha(np.repeat(np.linspace(0,1,cbar_size).reshape(1,-1),cbar_height,0), bytes=True)
    ...
    d.text((cbar.size[0]-180,15), '≥'+str(MAX), font=cbar_font, fill=(255,255,255,255))
    ...
    border = cm.gray(np.repeat(np.zeros(scan_summary.shape[1]).reshape(1,-1),50,0),bytes=True)

    border = Image.fromarray(border)
    d = ImageDraw.Draw(border)
    d.text((20,15), str(display_path_name), font=scan_info_font, fill=(255,255,255,255))
    if image_date:
        d.text((border.size[0]-340,15), f'PET date: {image_date}', font=scan_info_font, fill=(255,255,255,255))
...
def draw_mridependent(output,images,tracer,xnat_paths=False,image_dates=False,include_metadata=True):
    '''
   ...
   # brainmask_data = filter_gaussian(brainmask_data,aparc_img.affine,[12,12,12])
    ...
    slice_xyz,cropped_voxels = findCropSlice([brainmask,],pad=14)
    ...
    QC_cortex_outline = loadMask(aparc_img,rgba=(255,255,255,255),indicies=HEMISPHERE_OUTLINE_CODES)

    # Create reference region colored images
    if tracer in INFCEREB:
        reference_region_img_filled = loadMask(infcereb_img,rgba=(255,30,30,140),indicies=1)
        reference_region_img_outline = loadMask(infcereb_img,rgba=(255,10,10,255),indicies=1)
    elif tracer in WCEREB:
        reference_region_img_filled = loadMask(aparc_img,rgba=(255,30,30,140),indicies=WHOLE_CEREB_CODES)
        reference_region_img_outline = loadMask(aparc_img,rgba=(255,10,10,255),indicies=WHOLE_CEREB_CODES)
    elif tracer in CEREBGM:
        reference_region_img_filled = loadMask(aparc_img,rgba=(255,30,30,140),indicies=CEREB_GM_CODES)
        reference_region_img_outline = loadMask(aparc_img,rgba=(255,10,10,255),indicies=CEREB_GM_CODES)
    else:
        print('ERROR Error error')
        print('Tracer (', tracer,')','is of unknown type!' )
        return False
    ...
    # Create primary ROI colored images
    if tracer in METAROI_TRACERS:
        primary_roi_filled = loadMask(aparc_img,rgba=(255,0,0,180),indicies=META_TEMPORAL_CODES)
        primary_roi_outline = loadMask(aparc_img,rgba=(255,255,255,255),indicies=META_TEMPORAL_CODES)
    elif tracer in CORTSUM_TRACERS:
        primary_roi_filled = loadMask(aparc_img,rgba=(255,0,0,180),indicies=CORTICAL_SUMMARY_CODES)
        primary_roi_outline = loadMask(aparc_img,rgba=(255,255,255,255),indicies=CORTICAL_SUMMARY_CODES)
    else:
        print('ERROR Error error')
        print('Tracer (', tracer,')','is of unknown type!' )
        return False
    ...
    scan_summary = np.concatenate([axial_images,],axis=0)

    ############################################
    #              Section 3
    #   Draw A/S/C MRI with masks ontop
    ############################################
    def draw_middle_rows(Slices,Display_Slices,mri,cortex_outline,reference_roi,axis):
        image_singles = []
        for slice_ in Slices:
            mri_frame = draw_image(mri.take(slice_,axis=axis),flip_LR=True)

            cortex_outline_frame = draw_mask(cortex_outline.take(slice_,axis=axis),outline_value=1,flip_LR=True)
            mri_frame.alpha_composite(cortex_outline_frame,(0,0))

            reference_roi_frame = draw_mask(reference_roi.take(slice_,axis=axis),outline_value=0,flip_LR=True)
    ...
    r1 = draw_middle_rows(Axial_Slices,
                          Display_Axial_Slices,
                          nu_grey_img,
                          QC_cortex_outline,
    ...
    r2 = draw_middle_rows(Coronal_Slices,
                          Display_Coronal_Slices,
                          nu_grey_img,
                          QC_cortex_outline,
                          reference_region_img_filled,
                          axis=1)
    r3 = draw_middle_rows(Sagittal_Slices,
                          Display_Sagittal_Slices,
                          nu_grey_img,
                          QC_cortex_outline,
                          reference_region_img_filled,
    ...
    def draw_bottom_rows(Slices,Display_Slices,mri,pet,cortex_outline,reference_roi,axis):
        image_singles = []
        for slice_ in Slices:
            mri_frame = draw_image(mri.take(slice_,axis=axis),flip_LR=True)
            ...
            reference_roi_frame = draw_mask(reference_roi.take(slice_,axis=axis),outline_value=2,flip_LR=True)
            mri_frame.alpha_composite(reference_roi_frame,(0,0))
    ...
    if image_dates:
        d.text((border.size[0]-340,15), f'MRI date: {image_dates[0]}', font=scan_info_font, fill=(255,255,255,255))
        d.text((border.size[0]-340,55), f'PET date: {image_dates[1]}', font=scan_info_font, fill=(255,255,255,255))
    border = np.asarray(border)
...
def draw_fullAtlas(output,nu,aparc,pet,scale,xnat_paths=False,image_dates=False):
   ...
   # brainmask_data = filter_gaussian(brainmask_data,aparc_img.affine,[8,8,8])
    brainmask = nib.Nifti1Image(brainmask_data, aparc_img.affine, aparc_img.header)
    slice_xyz,cropped_voxels = findCropSlice([brainmask,],pad=5)
    ...
    fs_atlas = loadAparc(aparc_img,alpha=.65)
    ...
    applicable_slices = np.linspace(0,image_dimensions[2]-1, columns+extra,dtype=int)
    Axial_Slices = np.sort(applicable_slices)[end:-end]
    Display_Axial_Slices = Axial_Slices + cropped_voxels[2]

    applicable_slices = np.linspace(0,image_dimensions[1]-1, columns+extra,dtype=int)
    Coronal_Slices = np.sort(applicable_slices)[end:-end]
    Display_Coronal_Slices = Coronal_Slices + cropped_voxels[1]

    applicable_slices = np.linspace(0,image_dimensions[0]-1, columns+extra,dtype=int)
    Sagittal_Slices = np.sort(applicable_slices)[end:-end]
    Display_Sagittal_Slices = Sagittal_Slices + cropped_voxels[0]
...
def draw_mridependent_TylerToueg(output,nu,aparc,names=False):
    ...
    # brainmask_data = filter_gaussian(brainmask_data,aparc_img.affine,[12,12,12])
    brainmask = nib.Nifti1Image(brainmask_data, aparc_img.affine, aparc_img.header)
    slice_xyz,cropped_voxels = findCropSlice([brainmask,],pad=10)
    ...
    scan_summary = np.vstack((border,scan_summary))
    border = cm.gray(np.repeat(np.zeros(scan_summary.shape[1]).reshape(1,-1),20,0),bytes=True)
